...
const TickerDataTable = () => {
  const [activeTickerSymbol, setActiveTickerSymbol] = useState(tickerTabs[0].symbol)
  ...
  const fetchTickerData = useCallback(async () => {
    ...
    try {
      ...
      const fetchResponse = await fetch(
        `/api/ohlcv/${activeTickerSymbol}?data_interval=1d&start_date=${startDate}&end_date=${endDate}`,
      )
      if (!fetchResponse.ok) {
      ...
      const processedData = jsonResponse.data
...
export default TickerDataTable // You might want to rename this to MainTable as well
