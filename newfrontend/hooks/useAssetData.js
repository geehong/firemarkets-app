...
import { getAssetDetail, getAssetOhlcv } from "@/lib/assets"

export default function useAssetData(assetIdentifier, options = {}) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [d, c] = await Promise.all([
        getAssetDetail(assetIdentifier),
        getAssetOhlcv(assetIdentifier, options),
      ])
      setDetail(d)
      setOhlcv(c)
    } catch (e) {
  ...
  }, [assetIdentifier, options])

  useEffect(() => {
    load()
  }, [load])

  return { detail, ohlcv, loading, error, reload: load }
}
