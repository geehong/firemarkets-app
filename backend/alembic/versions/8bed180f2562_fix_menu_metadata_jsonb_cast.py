"""fix_menu_metadata_jsonb_cast

Revision ID: 8bed180f2562
Revises: 1e4cb05459f1
Create Date: 2026-01-28 09:25:00.000000
...
revision = '8bed180f2562'
down_revision = '1e4cb05459f1'
branch_labels = None
depends_on = None


def upgrade():
    # Explicitly cast text to jsonb for menu_metadata
    op.execute("""
    ...
    BEGIN
        ...
        IF assets_menu_id IS NULL THEN
            ...
            VALUES ('Assets', 'cibGoldenline', 20, 'static', '{"description": "자산 분석 및 관리", "permissions": ["user", "admin"], "badge": null}'::jsonb) 
        ...
        IF onchain_menu_id IS NULL THEN
            INSERT INTO menus (name, icon, "order", source_type, menu_metadata) 
            VALUES ('OnChain', 'cibBitcoin', 30, 'static', '{"description": "온체인 데이터 분석", "permissions": ["user", "admin"], "badge": "NEW"}'::jsonb) 
        ...
        IF map_menu_id IS NULL THEN
            INSERT INTO menus (name, icon, "order", source_type, menu_metadata) 
            VALUES ('Map', 'cilChartPie', 40, 'static', '{"description": "지도 및 시각화", "permissions": ["user", "admin"], "badge": null}'::jsonb) 
        ...
        IF assets_menu_id IS NOT NULL THEN
            ...
            VALUES (
                ...
                5,
                'dynamic',
                '{"description": "전체 자산 보기", "permissions": ["user", "admin"], "badge": null}'::jsonb
            );
            ...
            VALUES (
                ...
                'dynamic',
                ('{"description": "' || asset_type_record.type_name || ' 자산 분석", "permissions": ["user", "admin"], "badge": null}')::jsonb
            );
        END LOOP;
        END IF;
        ...
        LOOP
            ...
            VALUES (
                ...
                'dynamic',
                ('{"description": "' || onchain_category_record.category || ' 메트릭 분석", "permissions": ["user", "admin"], "badge": null}')::jsonb
            ...
            LOOP
                ...
                VALUES (
                    ...
                    'dynamic',
                    ('{"description": "' || onchain_metric_record.name || ' 분석", "permissions": ["user", "admin"], "badge": null}')::jsonb
                );
            END LOOP;
        END LOOP;
        ...
        IF NOT EXISTS (SELECT 1 FROM menus WHERE name = 'Performance Map' AND parent_id = map_menu_id) THEN
            INSERT INTO menus (name, path, parent_id, "order", source_type, menu_metadata)
            VALUES ('Performance Map', '/overviews/treemap', map_menu_id, 10, 'static', '{"description": "성과 지도 시각화", "permissions": ["user", "admin"], "badge": null}'::jsonb);
        END IF;
        
        IF NOT EXISTS (SELECT 1 FROM menus WHERE name = 'World Assets TreeMap' AND parent_id = map_menu_id) THEN
            INSERT INTO menus (name, path, parent_id, "order", source_type, menu_metadata)
            VALUES ('World Assets TreeMap', '/world-assets-treemap', map_menu_id, 20, 'static', '{"description": "전 세계 자산 트리맵", "permissions": ["user", "admin"], "badge": null}'::jsonb);
        END IF;
