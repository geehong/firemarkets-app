#!/usr/bin/env python3
"""
Unnecessary Funds Cleanup Script
...
사용법:
    python cleanup_unnecessary_funds.py [--dry-run] [--confirm]
...
class FundCleaner:
    ...
    def cleanup_unnecessary_funds(self, dry_run: bool = True):
        """불필요한 펀드들을 정리합니다"""
        print(f"\n🔍 {'DRY-RUN 모드' if dry_run else '실제 삭제 모드'}로 불필요한 펀드 정리 시작...")
        print("=" * 60)
        print("✅ 유지할 중요 펀드들:")
...
def main():
    parser = argparse.ArgumentParser(description='불필요한 펀드 정리 스크립트')
    ...
    print("🧹 Unnecessary Funds Cleanup Script")
