  ...
  return (
    <header className="hidden lg:flex sticky top-0 w-full bg-white border-gray-200 z-99999 dark:border-gray-800 dark:bg-gray-900 lg:border-b" suppressHydrationWarning>
      <div className="flex items-center w-full px-1 py-1 lg:px-1 lg:py-1">
        ...
        {/* 모바일 메뉴 (햄버거 메뉴가 열렸을 때) */}
        {isMobileOpen && (
          <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 lg:hidden">
            <nav className="px-4 py-2 space-y-1">
              {navItems.map((item, index) => (
                <Link
                  key={index}
                  href={item.path}
                  className="block px-3 py-2 text-sm font-medium text-gray-700 rounded-md hover:bg-gray-100 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-white transition-colors duration-200"
