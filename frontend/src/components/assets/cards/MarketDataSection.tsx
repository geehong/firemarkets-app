...
import { formatDate, formatNumberWithLocale, toNumber } from '@/utils/formatters'
...
const MarketDataSection: React.FC<MarketDataSectionProps> = ({ asset, commonData }) => {
    ...
    return (
        <div>
            <h4 className="font-medium mb-2 text-gray-900 dark:text-gray-100">Market Data</h4>
            <div className="space-y-2 text-sm text-gray-700 dark:text-gray-300">
                {prevClose !== null && (
                    <div className="flex justify-between">
                        <span className="text-gray-500 dark:text-gray-400">Previous Close:</span>
                        <span className="font-medium">{formatCurrency(prevClose)}</span>
                    </div>
                )}
                    ...
                    <>
                        {week52High !== null && (
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium">{formatCurrency(week52High)}</span>
                            </div>
                        )}
                        {week52Low !== null && (
                            <div className="flex justify-between">
                                <span className="text-gray-500 dark:text-gray-400">52 Week Low:</span>
                                <span className="font-medium">{formatCurrency(week52Low)}</span>
                            </div>
                        )}
                    </>
                )}
                {volume !== null && (
                    <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatNumberWithLocale(volume)}</span>
                    </div>
                )}
                {averageVol3m !== null && (
                    <div className="flex justify-between">
                        <span className="text-gray-500 dark:text-gray-400">Avg Volume (3M):</span>
                        <span className="font-medium">{formatNumberWithLocale(averageVol3m)}</span>
                    </div>
                )}
                {day50MA !== null && (
                    <div className="flex justify-between">
                        <span className="text-gray-500 dark:text-gray-400">50 Day MA:</span>
                        <span className="font-medium">{formatCurrency(day50MA)}</span>
                    </div>
                )}
                {day200MA !== null && (
                    <div className="flex justify-between">
                        <span className="text-gray-500 dark:text-gray-400">200 Day MA:</span>
                        <span className="font-medium">{formatCurrency(day200MA)}</span>
                    </div>
                )}
                {lastUpdated && (
                    <div className="flex justify-between">
