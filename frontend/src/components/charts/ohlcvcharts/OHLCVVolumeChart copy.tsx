...
const OHLCVVolumeChart: React.FC<OHLCVVolumeChartProps> = ({
  ...
  const { data: delayedData, isLoading: delayedLoading, error: delayedError } = useDelayedQuotes(
    ...
    { enabled: !!assetIdentifier && isIntradayInterval && selectedInterval === '15m', staleTime: 60_000, retry: 3 }
  )
        ...
        chart: {
          events: {
            load: function (this: any) {
              // Remove stock tools from DOM after chart loads
              setTimeout(() => {
                const selectors = [
                ...
                // Also remove from chart container
                if (this.container) {
                  const stockToolsElements = this.container.querySelectorAll('[class*="stocktools"], [class*="bindings"], [class*="menu-wrapper"]')
                  stockToolsElements.forEach((el: Element) => {
                    try {
                      if (el.parentNode) {
                        el.parentNode.removeChild(el)
                      }
  ...
  // Remove stock tools from DOM after chart is rendered
  useEffect(() => {
    if (!chartRef.current?.chart) return

    const removeStockTools = () => {
      ...
      setTimeout(() => {
        ...
        // Also try to find and remove by class name in chart container
        const chartContainer = chartRef.current?.chart?.container
        if (chartContainer) {
          const stockToolsElements = chartContainer.querySelectorAll('[class*="stocktools"], [class*="bindings"], [class*="menu-wrapper"]')
          stockToolsElements.forEach(el => {
            if (el.parentNode) {
              el.parentNode.removeChild(el)
            }
          })
    ...
    // Remove on chart load
    const chart = chartRef.current.chart
    if (chart) {
      Highcharts.addEvent(chart, 'load', removeStockTools)
      Highcharts.addEvent(chart, 'render', removeStockTools)
      removeStockTools() // Also try immediately
    }

    return () => {
      if (chart) {
        Highcharts.removeEvent(chart, 'load', removeStockTools)
        Highcharts.removeEvent(chart, 'render', removeStockTools)
      }
    }
  ...
  useEffect(() => {
    ...
    const links = [
      // loadCSS('https://code.highcharts.com/css/stocktools/gui.css'), // Disabled - stock tools not used
    ...
    style.textContent = `
      ...
      /* Hide stock tools completely - more specific selectors */
      div.highcharts-stocktools-wrapper,
      div.highcharts-bindings-container,
      div.highcharts-bindings-wrapper,
      div.highcharts-menu-wrapper,
      ul.highcharts-stocktools-toolbar,
      ul.stocktools-toolbar,
      .highcharts-stocktools-wrapper *,
      .highcharts-bindings-container *,
      .highcharts-bindings-wrapper *,
      .highcharts-menu-wrapper *,
    ...
    <div style={{ background: '#ffffff' }}>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          <label htmlFor="interval-select" style={{ 
            color: '#666666', 
            fontSize: '14px',
            margin: 0
          }}>
          ...
          >
            {intervalOptions.map(option => (
              <option key={option} value={option} style={{ background: '#ffffff', color: '#333333' }}>
      ...
      />
      <p className="highcharts-description" style={{ color: '#666666', margin: '0.3rem 10px' }}>
        Chart with candlesticks showing {assetIdentifier || 'AAPL'} price with Ichimoku indicators and volume below on a separate axis.
      </p>
    </div>
  )
}
