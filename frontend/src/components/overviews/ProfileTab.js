...
const ProfileTab = ({ asset, stockProfile, stockFinancials, etfInfo, cryptoMetrics, ohlcvData }) => {
  ...
  const getCompanyDescription = () => {
    if (asset?.type_name === 'Stocks') {
      return stockProfile?.description || 'Detailed company information will be displayed here.'
    } else if (asset?.type_name === 'ETFs') {
      return etfInfo?.description || 'Detailed ETF product description will be displayed here.'
    } else if (asset?.type_name === 'Crypto') {
      return (
        cryptoMetrics?.description || 'Detailed cryptocurrency information will be displayed here.'
      )
    }
  ...
  const getBusinessInfo = () => {
    if (asset?.type_name === 'Stocks' && stockProfile) {
      return {
        ceo: stockProfile.ceo || 'N/A',
        employees: stockProfile.employees_count || 'N/A',
        ipoDate: stockProfile.ipo_date || 'N/A',
        country: stockProfile.country || 'N/A',
        city: stockProfile.city || 'N/A',
        address: stockProfile.address || 'N/A',
        phone: stockProfile.phone || 'N/A',
        website: stockProfile.website || 'N/A',
      }
    } else if (asset?.type_name === 'ETFs' && etfInfo) {
      return {
        ceo: 'N/A',
        employees: 'N/A',
        ipoDate: etfInfo.inception_date || 'N/A',
        country: etfInfo.country || 'N/A',
        city: etfInfo.city || 'N/A',
        address: etfInfo.address || 'N/A',
        phone: etfInfo.phone || 'N/A',
        website: etfInfo.website || 'N/A',
      }
    } else if (asset?.type_name === 'Crypto' && cryptoMetrics) {
      return {
        ceo: 'N/A',
        employees: 'N/A',
        ipoDate: cryptoMetrics.launch_date || 'N/A',
        country: cryptoMetrics.country || 'N/A',
        city: 'N/A',
        address: 'N/A',
        phone: 'N/A',
        website: cryptoMetrics.website || 'N/A',
      }
    }
    return {
      ceo: 'N/A',
    ...
    if (asset?.type_name === 'Stocks' && stockFinancials) {
      ...
      return {
        marketCap: financialData.market_cap
          ...
          : 'N/A',
        prevClose: financialData.close_price ? financialData.close_price.toFixed(2) : 'N/A',
        prevVolume: financialData.volume
          ? `${(financialData.volume / 1000000).toFixed(3)}M`
          : 'N/A',
