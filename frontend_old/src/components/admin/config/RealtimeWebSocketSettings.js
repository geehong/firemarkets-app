...
const RealtimeWebSocketSettings = () => {
  ...
  const realtimeSettings = localRealtimeSettings || groupedConfigurations['realtime_settings']
  const websocketConfig = localWebSocketConfig || groupedConfigurations['websocket_config']
    ...
    <div>
      ...
      {realtimeSettings && (
        <>
          ...
          <CRow>
            {Object.entries(realtimeSettings.config_value).map(([key, item]) => (
      ...
      {websocketConfig && (
        <>
          ...
          <CRow>
            {Object.entries(websocketConfig.config_value).map(([key, item]) => (
              <CCol xs={12} md={6} key={key} className="mb-4">
                <CCard>
                  <CCardHeader>
                    <CCardTitle className="d-flex justify-content-between align-items-center">
