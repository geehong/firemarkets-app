...
const Widgets = () => {
  ...
  useEffect(() => {
    const fetchTopAssets = async () => {
      try {
        const response = await axios.get('/api/v1/assets?limit=10&has_ohlcv_data=true')
  ...
  return (
    <>
      ...
      <CCard className="mb-4">
        <CCardHeader>Top Assets - Real Time Prices</CCardHeader>
        <CCardBody>
          {loading ? (
            <p>Loading assets...</p>
          ) : (
            <CRow xs={{ gutter: 4 }}>
              {topAssets.slice(0, 8).map((asset, index) => (
                <CCol key={asset.asset_id} xs={12} sm={6} xl={3}>
                  <CWidgetStatsB
                    ...
                    progress={{
                      ...
                      value: Math.abs(asset.change_percent_24h || 0),
                    }}
                  />
                </CCol>
              ))}
